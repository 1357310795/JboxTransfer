namespace JboxTransfer.Core.Modules
{
    public class CRC64
    {
        ...
        public void TransformBlock(byte[] inputBuffer, int inputOffset, int inputCount)
        {
            ...
            {
                uint tableIndex = ((uint)(value ^ inputBuffer[inputOffset + i])) & 0xff;
                value = s_CRC64Table[tableIndex] ^ (value >> 8);
            }
            value = ~value;
        }

        public ulong TransformFinalBlock()
        {
            return value;
        }
